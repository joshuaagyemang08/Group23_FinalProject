...
def get_neighbors(drug):
    ...
    for _, row in ddinter_df.iterrows():
        if row['Drug_A'] == drug and row['Level'] == 'Major':
            neighbors.append((row['Drug_B'], row['Level']))
        elif row['Drug_B'] == drug and row['Level'] == 'Major':
...
def graph():
    ...
    red_patch = mpatches.Patch(color='red', label='Major')
    plt.legend(handles=[green_patch, orange_patch, red_patch], loc='upper left')
